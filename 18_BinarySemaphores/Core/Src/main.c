...
TaskProfiler RedLEDProfiler, BlueLEDProfiler, YellowLEDProfiler;
...
void redLedControllerTask(void *pvParameters)
{
	//used to avoid deadlock cuz at least one task needs to take up the semaphore before it can be passed around
	xSemaphoreGive(xBinarySemaphore);
	while(1)
	{
		xSemaphoreTake(xBinarySemaphore,portMAX_DELAY);
		RedLEDProfiler++;
		//if the below line was commented out the code should halt after 1 execution of red
		xSemaphoreGive(xBinarySemaphore);
	}
}
